    ...
    public class PiChildElementsClient
    {
        ...
        private async Task<string?> TryGetScadaWellStatusAsync(ChildElement child, CancellationToken ct)
        {
            try
            {
                ...
                var attrListUrl =
                    $"elements/{child.WebId}/attributes?nameFilter={encodedName}&selectedFields=Items.WebId;Items.Name;Items.Links.Value";
                ...
                using var valResp = await _http.GetAsync(attr.Links.Value, ct);
                ...
                //var tv = JsonSerializer.Deserialize<String>(valJson, _jsonOptions);
                //Console.WriteLine(tv);
                //var Finalvalue = tv;
                //Console.WriteLine(Finalvalue);

                using var doc = JsonDocument.Parse(valJson);
                string scanStatus = doc.RootElement
                                       .GetProperty("Value")
                                       .GetProperty("Name")
                                       .GetString();

                //Console.WriteLine(scanStatus); // "Scan Off"
